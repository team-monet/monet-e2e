...
| 2026-08-12 (run 11) | 15 | 100 | 0 | 1..15 all | Run 11: test15 (contradiction DISMISS path) added — dismiss is a pure no-verdict close: works WITHOUT body even on 2+ prior-observation concepts (contrast: accept-new refuses), closes contradiction (row retained, status='dismissed'), leaves body/obs/revisions untouched (0 concept_revisions rows vs 1 for accept-new); verdict matrix complete; test09 burst flake observed (5/20 stores fail + server death under 4x model load) — instrumented with per-writer exception + stderr surfacing, not reproducible standalone; assertions 160 → 176 |
