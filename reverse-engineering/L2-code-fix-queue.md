...
| RE-53 | S3 | A store that crosses the ambiguity gate (#86 third outcome, below `tauMargin`) is counted by NO resolution event — the ask throws before `recordResolutionEvent` (transaction already rolled back; `resolution_events.observation_id` NOT NULL), and the retry's `direct-attach`/`force-new` are excluded from `DECIDED_RESOLUTION_MODES`. Together they drop the whole exchange, biasing every rate ÷ `decidedTotal` (fork/duplicate-emission) toward exactly the population the gate shrinks (upstream #88, Codex review on #87). | Schema change for the observation-less ask row + a caller-supplied retry signal on `memory_store` (wider than #87) |
| RE-54 | S2 | The 12 `packages/core/scripts/measure-*.ts` reimplement `scoreNativeConceptsByObservation`/`resolveIncoming` instead of calling them (0/12 + 1/12) — every constant they justify (`tauAttach`/`tauMargin`/`edgeSimMin`/`NATIVE_SCORE_FLOOR`/`LEXICAL_BOOST`/`reliableSegmentTokens`) drifts toward looking derived; Codex found 11 divergences across 4 rounds, all numerically inert by luck; STARTER_SUITE cannot exhibit the probe-unit defect (upstream #89, **John-ratified 2026-08-23**). | A shared driver calling the engine's scorer+decision (`scoreNativeConceptsByObservation`/`resolveIncoming` both module-safe), then re-derive constants per profile |
