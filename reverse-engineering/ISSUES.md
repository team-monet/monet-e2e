...
| RE-04 | Lexical rank arm is Latin-script-only (`/[a-z0-9][a-z0-9_-]{2,}/`) → Korean/Japanese queries get zero lexical contribution. **[FIXED in 1.9.0 upstream #38/#97]** — the CJK bigram lexical arm (`\p{L}\p{N}` bigram class; `observation_tokens` re-indexed via `lexical_tokens_version`) now generates Korean postings; verified 2026-08-27 on 1.9.0: a Korean store produces 17 lexical tokens (was 0) and Korean semantic retrieval hits — test30 flips XFAIL→XPASS (regression guard, committed) | search-pipeline.md | fixed | test30 | S3 |
...
| RE-21 | graphDensity includes possible_duplicate_of edges, slightly inflating "structural density" | dashboard.md | fixed | test27 (XPASS) | S4 |
...
| RE-55 | `memory_declare` writes a principle into an ARCHIVED circle with NO disclosure: it is an ordinary store, so it does not consult `isArchivedCircle`, and the receipt `{action, advisories, circle, conceptId, species}` carries no `archived`/`guidance`/`landedInArchivedCircle` signal — a declared rule/principle is an ordinary concept and genuinely falls out of store-wide search when archived, yet the caller is told the declaration succeeded. #101 (which fixed the checkpoint sibling RE-51) explicitly names declare() and reassignCircle as REMAINING undisclosed paths (upstream #101 note). Confirmed 2026-08-30 on 1.9.1: store anchor → archive circle → `memory_declare`{species: principle} → receipt keys without archived signal; DB `concepts` row `kind='principle'` lands in the archived circle name. Desired contract (test56 XFAIL): refuse, or disclose the archived landing. **[1.10.0 re-check 2026-09-01: still XFAIL** — #128 homeCircle (47d48567) names only the circle a rule/skeleton member is HOMED in for cross-breadth delivery (`*`/global), and does NOT touch the archived-declare disclosure path; test56 flakes nothing → RE-55 unresolved in 1.10.0] | checkpoint-circle.md | confirmed | test56 | S3 |
| RE-56 | `memory_reassign_circle` moves a concept into an ARCHIVED circle with no archived-landing disclosure: the only archived intervention is `assertArchivedCircleMoveAllowed` (engine.ts:4165), which refuses ONLY circle-local live blocking RULES moved into an archived destination — every other concept (incl. a principle/preference without a live blocking rule) moves freely, and the receipt `{action, conceptId, fromCircle, toCircle, observationsMoved}` names the destination circle but carries no `guidance`/`archived`/`landedInArchivedCircle` clause saying the row now sits OUTSIDE store-wide recall. #101 (which fixed the checkpoint sibling RE-51) explicitly names reassignCircle as a REMAINING undisclosed path alongside declare() (RE-55). The move itself is intended (reassigning a concept into an archived circle is a legitimate way to shelve it) — the gap is disclosure parity with the #101 checkpoint fix. Structural: the archive-move already has the assert door, so the disclosure gap is entangled with it (no clean separate MCP behavioral surface) → L2 (S3) **[1.10.0 re-check 2026-09-01: UNRESOLVED** — the reassignCircle handler on `main` (mcp-server.ts:2162-2308) still emits `{action, conceptId, fromCircle, toCircle, observationsMoved}` with a `DENY RELOCATED` clause but NO archived/`landedInArchivedCircle` disclosure when `toCircle` is archived; #128 homeCircle doesn't touch this path → RE-56 stays `source`, L2] | circle-routing.md | source | — | S3 |
| RE-57 | `monet doctor --json` drops `startupFailure` on the FAILURE path — the one path where the record matters: on dist 1.11.0 a corrupt store (`monet.db` = not-sqlite) makes `start` die closed and write `monet.db.startup-failure.json` (`phase: "store-open"`, `error.code: "SQLITE_NOTADB"`), the TEXT doctor surfaces it ("last recorded startup failure"), but the same invocation's `--json` envelope (`monet.recovery.v1`) returns only `{schema,command,ok:false,dbPath,error:{RepairOperationError "not-sqlite"},inspection,provider,nextCommands,backup,report}` with **no** `startupFailure` key — so a CI/operator reading the machine-readable surface loses the causal startup record (the success path DOES carry `startupFailure:{"status":"none"}`) and RE-50's out-of-band diagnosis stays unreachable from `--json`. **[FIX MERGED upstream in `main`, NOT yet released: #120 closed 2026-09-14T02:35Z by PR #153 (commit 9fa38c2, `fix(cli): carry startupFailure into doctor --json on the failure path`)]** — desired contract (test57 XPASS): failure path carries `startupFailure.status=="found"` with `phase`/`error.code` intact, success path keeps `none`. Verified behaviorally on dist 1.11.0 (worker run 111); code anchors per upstream #120 (`repair-cli.ts:540/:567`, `printRecoveryError` `:1151`) are upstream-reported, not re-read by this run (GR-08) | repair-cli.md | confirmed | test57 | S3 |
| RE-58 | The engine constructor has **NO schema-version ceiling**: a store stamped ABOVE this build's supported schema is opened, bootstrapped and SERVED, while `monet repair` refuses the very same store. Upstream #107 records its failure mode as **"Inferred, not reproduced"** ("No fixture was built that stamps `user_version` above the ladder and opens the store, so the actual observed failure mode — which statement throws first, what the message says, and whether anything writes before it does — is not established here"); this run closes that gap on the shipped artifact. Measured on **dist 1.11.0** (2026-09-15, isolated temp stores, two arms): **(A)** a bare store stamped `user_version=14` with **zero tables** → `monet start` completes to `Monet started` with **no refusal**, the store-open bootstrap DDL creates **27 tables** (0→27) and flips the journal to WAL, and the agent then stores + searches normally (its `RE58-MARKER` row is returned by `memory_search`), surviving a second independent session; **(B)** a REAL seeded store, then stamped 14 → likewise opened, and the older build returns the pre-existing row to search. `user_version` stays **14** in both arms (the store keeps claiming a schema this build cannot name). **No statement throws at all**: the first write is the engine's own bootstrap DDL — the deferred failure #107 predicts is unreachable because the version ladder is skipped above the ceiling while the idempotent table-ensuring DDL runs unconditionally. `monet doctor` on the same store: `Schema: 14 (supported: 13)` + `Assessment: unknown` (rc 2); `monet repair --target …` REFUSES ("Store schema 14 is newer than supported schema 13; refusing repair.", rc 1) → the doctor/repair-vs-start **asymmetry reproduced behaviorally**. Desired contract (test58 XFAIL): `monet start` refuses a store newer than the build, writing nothing to it. Hazard framing: every later write into such a store is unsupported by construction (an older binary writing rows a newer schema may read differently) — but **no data loss was observed in this run**; the measured defect is the silent acceptance. The ceiling fix is PR #155, which is **OPEN and NOT merged** (direct PR-state read 2026-09-16: `state=OPEN`, `mergedAt=null`, HEAD `e81db7b`, base `main`, `mergeStateStatus=CLEAN`, updated 2026-09-15T00:49Z; **re-read 2026-09-17 run 116: still `state=OPEN`, `mergedAt=null`, HEAD advanced to `770cd98` — "keep the schema-version preflight side-effect-free on WAL stores", CI 4/4 green**; **re-read 2026-09-17 16:00 (run 117, direct `gh`, not carried forward): still `state=OPEN`, `mergedAt=null`, HEAD `770cd98` unchanged, base `main`, `mergeStateStatus=CLEAN`, 4/4 checks green — but the merge gate is now WITHHELD: the John-waived round-4 substitute review was consumed on this exact HEAD and its verdict is `findings` (2×P2 + 2×P3), so nothing merged and no further HEAD is authorized without a new human ruling**; flip-time checklist + measured fixture sidecar shapes for the new preflight in `schema-migration.md` RE-58 §Flip-time checklist) — so the ceiling is in neither `main` nor 1.11.0 (`main` HEAD `9fa38c2`; the shipped 1.11.0 bundle has 0 occurrences of `readStoredSchemaVersion` / `refusing to open` / `newer than this build`). `monet repair`'s own refusal text (`newer than supported schema`, 2 occurrences) is the only ceiling-side string present. The flip signal is the REFUSAL only — per upstream #156 the fixed CLI still writes the circle map before refusing, so the guard deliberately does not assert a write-free store. **Run 118 (2026-09-18): guard hardened to the run-117 constraints + the two uncovered shapes measured on the SHIPPED build** — test58 now carries FOUR arms (bare header / real seeded header / `-wal` without `-shm` / hot `-journal`); on dist 1.11.0 **all four are accepted AND served** (`verdict=xfail`, exit 2, zero throws) and each is bootstrapped in place (27 `CREATE TABLE`s; `user_version` stays 14 on every arm; the hot `-journal` is converted to WAL), i.e. write-freeness was never a property of the unfixed build on any shape; the accept/refuse decision is now one testable `verdict()` (xpass/xfail/**inconclusive**) and **exit code 4 = INCONCLUSIVE** is plumbed through `run_all.py` so a lock-contended probe cannot be read as a flip. **Run 121 (2026-09-19): #155 STATUS CHANGED — the run-116/117/118 claim "still `state=OPEN`, head `770cd98`" is SUPERSEDED. Direct read (G-5): `state=MERGED`, `mergedAt=2026-09-18T23:22:24Z`; `main` HEAD `9fa38c2` → `d920bc2` (merge = 4 files: core/storage.ts, core/engine.ts, 2 cli __tests__). Still in NO release (1.11.0 shipped 2026-09-01; shipped bundle literals `readStoredSchemaVersion`/`refusing to open`/`newer than this build` = 0), so RE-58 remains XFAIL on the published artifact and the flip contract is unchanged (a RELEASE carrying the fix, not a bump). Run 121 then did what no prior run could: it built the MERGED source and ran test58's four arms against it — `verdict=xpass`, exit 3, **all four arms refuse** ("Store schema 14 is newer than supported schema 13; refusing to open. Upgrade Monet first."), `user_version` unchanged 14 on every arm, and the two `null`-preflight stray shapes (orphan `-wal`, hot `-journal`) refuse too — so the next release carrying #155 should flip cleanly. Residual non-safety findings measured on the FIXED build: the refusal is NOT write-free (#156 behaviourally confirmed — `remote_circle_map`, WAL sidecars, `monet.db.startup-failure.json`), and the ceiling is checked AFTER the embedder phase (`openServedCore()` selects the provider in phase `embedder-selection`, then constructs the core in `store-open`), so an above-ceiling store still pays a full ~1 GB bge-m3 load (a cold-cache download) before being refused. Drift check: suite identical before/after the pull (38/60 PASS + 13 XFAIL + 8 XPASS + 1 STALE + 0 FAIL). **Run 122 (2026-09-20): the #156 half got its own fix HEAD — PR #159 (`agent-ops/monet-158-ceiling-shapes`, `Closes #158, Closes #156`) is `state=OPEN`/draft, HEAD `caa9d88`, `mergeable=MERGEABLE`, 4 checks; the ceiling half is still only in `main` (`d920bc2`), so the guard stays XFAIL on the published artifact and the flip contract is unchanged. This run built that HEAD from source and re-ran the four arms on the LIVE `monet start` path against it, HEAD vs merge base: the refusal is now **write-free at the main-file level** — `A_bare_header` sha-unchanged/0 B/`delete`/no `remote_circle_map` (base: changed/+8192 B/`delete→wal`/map created), `S2_hot_journal` `-journal` (4616 B) preserved (base: consumed), `S1_orphan_wal` WAL left unfolded with `uv 0` (base: folded to `uv 14` + `marker` + map) — leaving only the intended `monet.db.startup-failure.json` plus a 32 KB `monet.db-shm` on the orphan-`-wal` arm, and test58 reports `verdict=xpass` on that HEAD. Because the fix HEAD makes the refusal write-free, the guard's #156 caveat ('the fixed CLI still writes the circle map before refusing') can be tightened to a write-freeness assertion once a release carries it. The cost/attribution half is now **measured, not read from source order**: warm cache refusal 1.07 s vs empty `MONET_MODEL_CACHE` 33.43 s after downloading 586,779,294 B, while an accepted fresh-store start is 0.9 s → filed as **team-monet/monet#160** (read back `state=OPEN`), because PR #159 leaves `bootstrap.ts` byte-identical and so does not decide the ceiling before paying for the provider. A 0-byte `monet.db` was also measured: served and bootstrapped on 1.11.0, the merge base AND the fix HEAD (0 → 417,792 B) — the #158 defect is the pre-engine decision, not serving. Full record: schema-migration.md §"Pre-release flip validation against the MERGED fix" + §"Independent verification of the fix HEAD (run 122)".** (upstream #107/#155/#156/#159/#160, sev:major) | schema-migration.md | confirmed | test58 | S2 |
| RE-59 | An **unrecognised `writerRole`** converts a COMPLETE run into a reported LOSS. `parseMomentSpoolLine` rejects the run declaration against the writer roster (`"host-hook"` / `"gate-cli"` / `"core"`, moment-spool.ts:57-65 plus the `isWriterRole` gate at :757), the rejection is counted as `malformedLines` (:688/:705) and that count is **discarded by every production caller**; the fold then meets a run whose first readable record is `seq 1`, so it opens a **phantom `sequence-gap`** (`from_seq=0`, `to_seq=0`) in `moment_losses` that `gate.losses` counts — although every moment of that run folded (the interception behind the dropped declaration is served and appears in the overview's own accounting). Measured on **dist 1.11.0** in an isolated temp store (12 spool records / 3 runs) against the live `memory_overview` surface: the valid-role arm reports `losses=0`; the unknown-role arm reports `losses=1`. The loss row is attributable only through the `moment_runs` join, which carries NULL `writer_role`/`started_at` — so the "lost moment" is attributed to a line this build refused to READ, not to anything a writer lost. Sibling gaps, **not duplicates**: upstream **#68** is the INVERSE (a malformed line leaves `gate.losses` at ZERO — a silent loss), upstream **#72** is the future-version cursor freeze (`v` above the build parks the fold, no surface reports it); in all three the `MomentSpoolRead` counters (`malformedLines` / `futureVersionLines` / `restartedFromZero`) reach no durable surface, which is the shared root cause. **Blast radius inside our own suite:** `tests/test50_conformance_roundtrip.py:84` seeds `writerRole:"e2e"` — outside the roster — so the public conformance round-trip fabricates one loss per run and asserts nothing about `losses`. Filed **team-monet/monet#157** (read back `state=OPEN`). Desired contract (test60 XFAIL): a line this build cannot read must not be counted as a lost moment, and skipped lines must be disclosed. Domain note: the ledger has no dedicated RE doc yet (`render-overview.md` owns the served surface; `moment-ledger.ts`/`moment-spool.ts` still need one) | render-overview.md | confirmed | test60 | S3 |
| RE-60 | The **derived card TITLE (and the `slug` derived from it) is cut at the first period that is not sandwiched between digits**, so a memory that names a file, a domain or a version drops that name from every recall surface — and the same helper cuts the CONTRADICTION DETAIL an agent reads when arbitrating a correction. Mechanism: `firstLine()` = `t.trim().split(/\n\|(?<!\d)\.\|\.(?!\d)/)[0]` (shipped `dist/cli.js`; **the identical regex is still in `main` `d920bc2`** → not fixed upstream either). Measured on **dist 1.11.0** (isolated temp store, live MCP path, three observable surfaces): (1) **title/slug at store time** — `"Updated app/card-image.ts downloadCardImage canvas exporter for …"` → title `Updated app/card-image`, slug `updated-app-card-image`; `"Bump vite.config.ts target to es2022 for the … dashboard build"` → title `Bump vite`; `"Layout Height Collapse: Svelte mounts App.svelte at the root, …"` → title `Layout Height Collapse: Svelte mounts App`; `"Deploy the docs to example.ai and mirror … on acme.dev"` → title `Deploy the docs to example`; an email-led body → title `jane`; (2) **the search surface has no title at all** — `memory_search` cards are `{id, slug, kind, circle, observationCount}`, so for the recalling agent the slug IS the whole triage text (`bump-vite`); (3) **arbitration evidence** — `memory_store` correction ack `contradiction.detail = "correction: Correction: bump vite"` (`engine.ts:5772` `correction: ${firstLine(content)}`), i.e. the agent asked to accept-new/keep-current cannot see that the correction changes the vite config; the same helper also feeds correction-as-evidence, impeachment and skeleton projections (task/statement titles derive differently and are unaffected). Controls that HOLD and must keep holding after a fix (regression arms): `0.6.0` (digits both sides) intact; a plain sentence intact; a real sentence end (`. ` / line end) still ends the title; `\n` still ends it. Upstream **#117** (author JohnOnLee, filed 2026-08-29, `state=OPEN`, 0 comments) carries the title surface and a suggested direction (`split(/\n\|\.(?=\s\|$)/)`); **no fix PR exists and no duplicate was filed**. Record-only tensions (evidence, deliberately NOT asserted): an abbreviation-led body (`"Use e.g. a lexicon entry …"` → `Use e`) is **not** covered by #117's suggested regex, so a test61 flip does not imply abbreviation support; and the email-led truncation is the same mechanism the issue itself flags as a privacy tension. Desired contract (test61 XFAIL): never cut inside a filename/domain token and keep the sentence-end + first-line semantics | title-derivation.md | confirmed | test61 | S3 |

| RE-61 | **The served identity is a frozen literal, not the release** — the MCP handshake is the first thing every host sees and it says `monet-core 0.7.0` on every 1.x build: `new McpServer({name:"monet-core", version:"0.7.0"}, …)` (`packages/core/src/mcp-server.ts:3869`, dist 1.11.0) with an in-repo assertion pinning the same string (`core/src/__tests__/lifecycle.test.ts:352`), so the number cannot drift with a release. Measured on the **live `monet start` stdio surface** (isolated stores, 1.11.0): `initialize` → `serverInfo = {name: monet-core, version: 0.7.0}` while the SAME install is `@team-monet/monet@1.11.0`, `monet --version` prints `1.11.0`, and the registry manifest of the same build (`packages/cli/server.json`) declares `"version": "1.11.0"`. The literal is byte-identical in the shipped `dist/cli.js` of five installed releases (`1.2.4`, `1.7.0`, `1.9.1`, `1.10.0`, `1.11.0`). Nothing else fills the gap: `monet doctor --json` carries no version field (16 keys) and `agent_context` returns only `{circle}`. Consequence: a host panel quoting `monet-core 0.7.0` cannot be mapped to a build (four minor lines behind, older than schema 13), and an agent inside a session cannot answer "which Monet version am I on?" from any tool response — only a human at a shell can. Upstream **team-monet/monet#161** (filed 2026-09-20, `state=OPEN`, no duplicate existed). Desired contract (test62 XFAIL): the handshake names the installed release, the value is not a frozen literal, and `dist` agrees with the build's own manifest | mcp-server.md | confirmed | test62 | S4 |
| RE-62 | Store resolution silently prefers `<project>/.monet` over the documented single `~/.monet` store: the rung order is `MONET_STORAGE_DIR` → `<projectDir>/.monet` if it exists → `$HOME/.monet`, with `projectDir = MONET_PROJECT_DIR \|\| CLAUDE_PROJECT_DIR \|\| cwd` resolved EXACTLY (no upward search) and no disclosure anywhere when the rung changes — so one ordinary `monet start -d <repo>/.monet` permanently changes the store every later bare invocation in that repo uses (bare `status` in a repo went `$HOME/.monet` concepts 1 → `<repo>/.monet` concepts 0, and the bare server then returned 0 hits for a memory that was just stored), an operator at `<repo>/sub` silently reads `$HOME/.monet` (concepts 0) while the agent host's data sits in `<repo>/.monet` AND that subdirectory call CREATES `$HOME/.monet/monet.db`, and the documented diagnosis path (`monet doctor --dir ~/.monet --json`, `packages/cli/README.md:50`) inspects a different DB than the server serves while `README.md:23` promises "One store at `~/.monet`" and `packages/cli/README.md:23` says projects are organized as *circles* inside it (no public doc mentions the rung or `MONET_PROJECT_DIR`). | storage.md | confirmed | **test64 (XFAIL, exit 2) + test63 (positive legs, PASS)** — suite-linked run 126: test64 REPRO 16/16 (H1 blind spot at `<proj>/sub` and `<proj>/sub/deep`; H2 silent flip), DESIRED 0/4; test63 holds the six consistent surfaces to the same file (44 checks, 0 failed). Replaces the run-125 `/tmp/r125_*` probes | S3 |
| RE-63 | A RELATIVE `MONET_STORAGE_DIR` is reported two different ways within one run: `status` stdout and `start` stderr print it verbatim (`Storage: rel-store/monet.db`) while `status` stderr and `doctor --json` print the resolved absolute path — the same surface (`status`) emits two strings for one store, so a consumer comparing `status` output with `doctor`/`config` output must resolve the path itself. | storage.md | confirmed | **test65 (XFAIL, exit 2)** — suite-linked run 126: 4 surfaces (status stdout/stderr, doctor `dbPath`, bare `start` stderr); adds the harm run 125 did not name — a reader resolving the verbatim string against its OWN cwd lands on a DIFFERENT file (`~/.monet-test/rel-store/monet.db` vs the served `<proj>/.monet/monet.db`) | S4 |
| RE-64 | **The rule-RATIFICATION surface (`memory_ratify`) had no MCP-side journey before run 127** — the 1.11.0 roster advertises the tool with four verdicts (`approve`/`re-ratify`/`reject`/`retire`), an `entrance` ("extraction" requires a `battery`, "declaration" forbids it), `memberRuleIds` and a `packet`, yet only the `retire`-inside-a-recipe leg had ever been driven live, so the entry verdicts, the derivation link and their persistence across a fresh process were unmeasured. Measured on **dist 1.11.0** (isolated temp store, live `monet start` stdio path, 71 checks, 0 failed; suite-linked as `test66`/scenario 13): every verdict + entrance rule + candidate guard + the derivation edge + the overview/agent_context projection hold end-to-end (declare → ratify → read back → **fresh process** read back). Four contracts worth not re-deriving: (1) the ack's `edgeIds` are `lifecycle_edges` ROW ids, NOT `memory_edge` rows and NOT the rule's concept id — the derivation link is `family='derivation'`, `src_concept_id`=principle → `dst_concept_id`=rule, `born_of='ratification'`, `event_ref`=the returned `ratificationId`; (2) derivation/provenance edges are **deliberately non-unique** (schema comment: "a principle derives many rules, and a rule corrected twice carries two evidence spans"), so the same rule named twice in ONE call yields one edge while approve-then-re-ratify yields two — de-duplication is per-call, not across verdicts; (3) `memory_resolve`'s PAIR-FLAG shape is `conceptAId`+`conceptBId` ONLY — passing `decision` (or `body`, or `contradictedObservationId`) alongside them is refused by name, and one dismissal clears BOTH directions of the row pair (`rowsUpdated=2`, pair edges are written bidirectionally); (4) `verdict="retire"` ends SKELETON MEMBERSHIP only (the tool description says so verbatim) — the concept stays live and `memory_retire` is still required, which then refuses a current member by naming the `memory_ratify … verdict="retire"` remedy. No defect found: every refusal named its remedy, and the only two red checks in the first draft were authoring errors against these four contracts. | mcp-server.md | confirmed | **test66 (PASS)** — suite-linked run 127: 71 checks green, including a 4-gate `extraction` battery, a declaration/`extraction` entrance boundary probe (both directions refused by name), `reject`-governs-across-reopen, and a retire → restore → re-ratify loop | S4 |
| RE-65 | **`memory_detach` was the LAST tool of the 1.11.0 roster with zero end-to-end evidence** — 21 tools are advertised and the only appearance of `memory_detach` anywhere in the suite was the roster string in `test01`, even though it is the single surface that can UNDO a wrong merge (the cleanup the server-ops playbook prescribes after a multilingual repair overmerges concepts: `references/attach-retest-and-overmerge-split.md`). Its description's four promises had never been measured on the wire: default `created` destination, `destConceptId` attach to an existing concept, "moving all observations into a destination deletes the source, consolidating a duplicate", and "the source is recomputed and marked for synthesis". Measured on **dist 1.11.0** (isolated temp store, live `monet start` stdio path; 53 checks, 0 failed; suite-linked as `test67` / scenario 14): all four hold, together with every scope/shape guard. Contracts worth not re-deriving — (1) a SPLIT takes the named observations **with their evidence**: `observationCount` drops, the source `body` is REBUILT without the moved text, and `needsSynthesis` RE-ARMS on BOTH sides; (2) the destination — newly created or named — receives the moved text, is flagged for synthesis, and keeps the source's `kind`; (3) a FULL consolidation (`sourceDeleted=true`) does not merely hide the source: its `concepts` row is GONE and `memory_fetch` on its id refuses, while the keeper holds every moved observation, re-serves the moved text on `memory_search`, and outranks the other holder of the same topic; (4) the deleted source's slug and id ARE carried as destination aliases in the DB — **but aliases are not id resolution**: `memory_fetch` by the removed slug OR the removed uuid is refused with `concept not found`, and `circleOf()` is id-only, so a caller holding only the old handle cannot reach the keeper through any tool param (aliases serve asserted `#slug` references). Refusal surface (13 checks): last-observation, self-destination, foreign/nonexistent/empty observation list, wrong-circle source, wrong-circle destination, workstream source, retired source — every refusal left BOTH sides untouched. No defect found; a fresh process re-read the whole journey. | mcp-server.md | confirmed | **test67 (PASS)** — suite-linked run 128: 53 checks green over six arms (split-to-new, split-to-existing, full consolidation, refusals, fresh-process durability) | S4 |
| RE-66 | **The test09 burst FAIL has a NAMED cause, and the test could not see it** — the 2026-08-12 record called this flake "resource contention (model-load memory pressure), not reproducible standalone", and the guard judged stores by `ok = bool(r.get("conceptId"))`, so a response that carried no id was counted as a failure with NO payload kept. Run 128 instruments that path and re-measures: the burst's failing store answers `{"_rawText": "store failed: database is locked"}` (raw text — no `conceptId`, no exception, no JSON-RPC error), and a concurrent `monet start` can fail to start at all (`phase: "store-open"`, `StoreBusyError`, `code: SQLITE_BUSY`, "SQLite reported it locked while a statement in the startup's schema region ran. That region had been running for 8179ms", 3,252 B record at `monet.db.startup-failure.json`; its clients then see `RuntimeError: server closed stdout / timeout` + `BrokenPipeError` and one instance stored only 10/20). The product's own error text names the boundary: **"One MCP server and one `monet` CLI call sharing a store is the supported topology"** — so burstA (4 servers × 5 stores on ONE store) asserts a topology the product declines to support: in-suite **2/2 FAIL** today vs standalone **6/7 PASS** (1/3 before instrumentation, 4/4 after), and it fails deterministically only when a second test09 runs concurrently (burst walls 23.1 s / 19.4 s vs 10–14 s healthy). The red is contention, not a regression; the check stays red on purpose. Dev Objective = the CONTRACT: state the supported topology on `status`/`doctor` and have the guard assert the DOCUMENTED shape (all-succeed, or the named `StoreBusyError`/`database is locked` — never a hang, corruption, or an id-less response), or add retry/backoff for startup + store under a shared store | storage.md | confirmed | test09 (FAIL, contention-dependent; instrumented run 128) | S2 |
