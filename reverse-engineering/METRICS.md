...
| 2026-08-16 | 16 (unchanged) — 3 docs cross-checked vs readable TS, 1 corrected for drift | 181 (unchanged) | 36 (unchanged) | Run 34 (DIRECTION priority #1): minified-doc drift cross-check. `search-pipeline.md` + `dedup-resolution.md` verified NO drift — every constant, the SQL, `MODEL_PROFILES`≡`pU`, `DEFAULT_MODEL`≡`uU`, and `applyEmbedderDerivedThresholds` are value-for-value identical vs `retrieval.ts`/`resolution.ts`/`lexical-overlap.ts`/`embedding-onnx.ts`/`engine.ts`. `schema-migration.md` had REAL drift: the ladder was refactored into NAMED version-gated migrations (`GRAPH=1…FIRST_BLOCK_RETIREMENT=12`); the doc's "no-op bump" labels (2→3/3→4/5→6/6→7) were wrong (2→3=AROUSAL is real version-gated DDL; 3→4/5→6/6→7 are named sentinels) and "DDL unguarded on every open" is outdated for temporal/arousal — corrected in-place. RE-09/RE-10 reconfirmed (`source`; version 10 still skipped). Added `L2-code-fix-queue.md` (11 confirmed bugs, severity-ordered, for John's prioritization). |
