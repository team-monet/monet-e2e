...
| 2026-08-19 (run 48) | 24 (unchanged — both are mcp-server.ts, already documented) | 284 (unchanged) | 50 (+2: RE-49, RE-50) | Upstream #59 + #13 triage (DIRECTION run #7 priorities #2/#3). **RE-49** (S3/open): `stage_lookup` clips body/reason and discards `clip()`'s `.clipped` flag — no `bodyTruncated`/`reasonTruncated` emitted, recovery path (`conceptId`→`memory_fetch`) exists but is undisclosed; E2E-verifiable → XFAIL test39 (flagged for E2E worker). **RE-50** (S2/source): `server.connect()` (mcp-server.ts:3491) is the first MCP-utterable moment; `ensureEmbedderPin()` + entry-point store-open/model-load all run before it, so every startup failure reads as "Connection closed" — a design decision (degraded mode vs out-of-band), routes to L2. Both verified against current `main` (line drift noted). |
| 2026-08-19 (run 50) | 24 (unchanged — E2E XFAIL conversion, no new module) | 284 (unchanged) | 50 (unchanged — RE-49 status flip) | RE-49 converted to XFAIL **test41** and CONFIRMED. Both DIRECTION run #7 RE priorities (#59→RE-49, #13→RE-50) were already triaged by run 48; with the read of the repo showing RE-49 still `open/pending-XFAIL` (neither RE run 48 nor E2E run 48/49 had written the XFAIL), this run wrote it. test41 (deterministic): declares a stage + an advisory rule whose body is 8100 chars (> STAGE_LOOKUP_BODY_CAP=6000) → `stage_lookup` → the clip fires (`...[truncated ...]` marker present, bodyLen 6021) but the delivered rule carries keys `{body, conceptId, origin, reason, reasonMissing, scope, severity, text}` with NO `bodyTruncated`/`reasonTruncated` → XFAIL (exit 2). `open` → `confirmed`, `e2e_test` `pending-XFAIL` → `test41`. Source cross-check vs `main` held (mcp-server.ts:146-150 clip, 1597-1625 handler discards `.clipped`, uses only `.text`); 1.6.3 installed binary reproduces identically. See diary 2026-08-19. |
| 2026-08-21 (run 58) | 24 (unchanged — no new module; coverage triage, no module doc) | 284 (unchanged) | 50 (unchanged — no new product issue) | **Coverage-gap triage by upstream-test evidence.** DIRECTION #3 (cli/remote-circle.ts 53.7%) proven a **false lead** on the run-56 question: `remote-circle.ts` (3 pure git-remote helpers) is exhaustively upstream-unit-tested — cli `__tests__/circle.test.ts` has 35 `canonicalRemoteKey` refs (SSH/HTTPS norm, scheme-specific default-port strip, non-default-port keep, `.git/` trailing-slash, SCP, folder fallback, getOriginRemote empty/throw, defaultNameFromRemote sanitize) + gate-cli.test.ts + install-cli.test.ts. Same bootstrap-style false lead → skip with rationale, reconcile monet-e2e#23. **Second (strategic) finding: raw coverage % has saturated (~84%)** — ran the same lens over all sub-85% modules; non-deprecated/non-halted/genuinely-NOT-upstream-tested gaps are only **cli/project-dir.ts (19L), core/embed-budget.ts (49L), core/resolution.ts (338L, the dedup engine — no symbol in any core test)**. Everything else is deprecated source\*/halted render-overview/upstream-tested. Recommends re-baselining the metric (report coverage over NOT-upstream-tested set only) and treating the behavioral XFAIL matrix as primary. New doc `coverage-triage.md`. |
| 2026-08-22 (run 63) | 24 (unchanged — maintenance reclassification, no new module) | 284 (unchanged) | 50 total (unchanged — RE-05/RE-25 → removed status flip, no new issue) | **Post-1.7.0 source-removal consistency cleanup (monet-e2e#25).** RE-05 (was `open`) and RE-25 (was `by-design`) reclassified `removed` — the open-source 1.7.0 release irreversibly hard-removed the source subsystem (CLI `source`, four `source_*` MCP tools, dashboard `/api/sources`; schema 12→13 retired the source tables), so both can no longer manifest. `e2e_test` set to their SKIP guardrails (RE-05→test26, RE-25→test25). sources-sync.md banner "PROVISIONALLY RETIRED" → **"HARD-REMOVED (1.7.0)"**; RE-05/RE-25 module sections annotated. This completes the source cleanup — RE-05/23/24/25/29/30 all `removed`, no source-subsystem issue active. RE-27 left untouched here (gates.md issue, already reassessed by-design in run 62 with test50 evidence). Impulse: active tracked issues 50→**48** (two moved to `removed`). |
| 2026-08-23 (run 66) | 26 (+2: checkpoint-circle, cli — issue-anchored module docs) | 284 (unchanged — no new threshold/param) | 52 total (RE-51, RE-52 confirmed in run 65 — 50→52) | **RE-side bookkeeping for the 08-21 upstream batch.** Run 65 (E2E) confirmed **RE-51** (`memory_checkpoint` writes into archived circle w/ NO disclosure, S2, test54 — sibling of RE-17) and **RE-52** (`monet install` crashes on malformed `hooks.PostToolUse*`, S3, test55) as XFAIL; this run created the two module docs the ISSUES rows point at — `checkpoint-circle.md` (wired `memory_checkpoint` handler, mcp-server.ts:1816-1853) + `cli.md` (install-cli.ts `validateSettingsShape` 1019-1043) — and promoted RE-51/RE-52 into the L2-code-fix-queue (active queue 12→14). Modules 24→26.
...
| Checkpoint / workstream-save surface (`memory_checkpoint` wired handler: `saveWorkstream`/`captureFind`/`previewWorkstreamCheckpoint`, `trackedCheckpointHandler`, touch history; archived-circle disclosure gap) | src/mcp-server.ts (+ engine.ts) | **DOCUMENTED** | `checkpoint-circle.md` |
| CLI install/settings-shape (`monet install`, `validateSettingsShape`, `buildCoverageReport`, RFC hook wiring; PostToolUse shape-validation gap) | src/cli/install-cli.ts | **DOCUMENTED** | `cli.md` |
