...
| GR-10 | A test that must call the CLI with NO `-d` (the bare-call storage-resolution class) MUST run every call under a sandboxed env (`HOME`, `MONET_PROJECT_DIR`, `CLAUDE_PROJECT_DIR`, `MONET_STORAGE_DIR`, `MONET_CIRCLE`, `MONET_CALLER_ID`) AND hard-assert that every path a surface reports is inside that sandbox BEFORE asserting the scenario — an out-of-sandbox path is a GR-01 violation and must abort the test loudly | 2026-09-22 | Coda | Run 126: the first RE-62 guard run left 4 bare `monet status` calls outside `process_env`, so they resolved `$HOME/.monet` — the REAL prod store — and opened it 4× (read paths only, no memory writes; main DB unmodified, WAL touched). `-d` is no defence for this test class, so the rule moves from "never run the CLI without `-d`" to "wrap the env and prove the sandbox" | Active |
| GR-11 | A FAILING CHECK IN A **NEW** TEST IS A HYPOTHESIS, NOT A FINDING: before writing a defect up, grep the product source for the keys the ack actually returns and for the refusal literal that fired, and re-derive the expectation from source — and a test helper MUST NEVER discard a tool response (an ignored `resp` is how a refusal is read as a silent success) | 2026-09-22 | Coda | Run 127: test66's first draft reported 5 red checks; all 5 were authoring errors against contracts the product states (ack `edgeIds` are `lifecycle_edges` ROW ids, not `memory_edge` rows; derivation edges are non-unique by design so one call mints one and a re-ratify mints two; `memory_resolve`'s pair path REFUSES `decision` — the `dismiss_dup_pairs` helper passed it AND threw the response away, so the dismissal silently never happened; a `reject` verdict ends membership, so the "still a member later" assertion was stale). Zero product defects; without the source read each red check becomes a filed issue. Corollary: pin the suite-log line too — `run_all.py` greps one `RESULT:` line, so a wrapped verdict loses everything after the newline (test66's RESULT was split over 2 lines until this run) | Active |
...
| 2026-09-22 | GR-10 added — bare-call tests must sandbox the env AND prove it | Run 126: 4 un-wrapped bare `monet status` calls resolved the real `$HOME/.monet` and opened the prod store | Coda |
| 2026-09-22 | GR-11 added — a red check in a NEW test is a hypothesis, not a finding; helpers must not discard responses | Run 127: all 5 red checks in test66's first draft were authoring errors against contracts the product states (`lifecycle_edges` row ids, by-design non-unique derivation edges, the pair path refusing `decision`, latest-verdict-governs) — 5 candidate issues, 0 real | Coda |
